...
import { PastSolutionByStrategy } from '@sudoku/stores/pastSolutionByStrategy';
import { CandidateGrid } from '@sudoku/stores/candidateGrid.js';

class StrategyService {
    constructor() {
        this.strategyServiceImpl = new StrategyServiceImpl();
        this.strategyNameList = ["NakedPair","X-Wing", "Y-Wing", "HiddenSingle", "NakedSingle"];
        // this.strategyNameList = ["HiddenSingle"];
        // this.strategyNameList = [];
        // this.strategyNameList = ["NakedSingle"];
        this.newestValidStrategyList = [];
    ...
    setHintStep(hint_step) {
        this.hintStep = hint_step;
    }
     ...
     */
    execute(grid) {
        // console.log(this.pastSolutionByStrategy.mergeMatrixAndStrings(grid, this.strategyNameList, this.hintStep));

        if (this.pastSolutionByStrategy.hasUserGrid(grid, this.strategyNameList, this.hintStep)) {
            const solution = this.pastSolutionByStrategy.getSolution(grid, this.strategyNameList, this.hintStep);
            return solution;
        }

        let candidate_grid = this.preprocessCandidateGrid(grid, this.initCandidateGrid);
        const executeReuslt = this.strategyServiceImpl.execute(grid, candidate_grid, this.strategyNameList, this.hintStep);
        const solution = executeReuslt.solution;
        this.newestValidStrategyList = executeReuslt.validStrategyList;
        this.pastSolutionByStrategy.addUserGrid(grid, structuredClone(this.strategyNameList), this.hintStep, solution);
    ...
    getNewestValidStrategyList () {
        return this.newestValidStrategyList;
    }
